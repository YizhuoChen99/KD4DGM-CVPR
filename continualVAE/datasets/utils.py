...
GenericLoader = namedtuple('GenericLoader',
                           'img_shp output_size train_loader test_loader')
...
def label_offset_merger(loaders, batch_size, use_cuda=False):
    ...
    # step 1
    max_labels_train = [
        find_max_label(loader.train_loader) for loader in loaders
    ]
    max_labels_test = [
        find_max_label(loader.test_loader) for loader in loaders
    ]
    ...
    for i in range(1, len(max_labels)):
        loaders[i].train_loader = _extract_and_increment(
            loaders[i].train_loader, i)
        loaders[i].test_loader = _extract_and_increment(
            loaders[i].test_loader, i)
...
def sequential_test_set_merger(loaders):
    test_datasets = [loaders[0].test_loader.dataset]
    for loader in loaders[1:]:
        current_dataset_clone = deepcopy(loader.test_loader.dataset)
        merged_dataset = loader.test_loader.dataset
        for td in test_datasets:
            merged_dataset += td

        # re-create the test loader
        # in order to get correct samples
        loader.test_loader = create_loader(
            merged_dataset,
            None,  #loader.test_loader.sampler,
            loader.test_batch_size,
            shuffle=True)
        test_datasets.append(current_dataset_clone)
